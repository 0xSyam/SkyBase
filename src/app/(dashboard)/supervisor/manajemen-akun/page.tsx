...
export default function SupervisorManajemenAkunPage() {
  ...
  const handleConfirmResetPassword = async () => {
    ...
    try {
      ...
      setNotification({
        type: "success",
        message: `Password ${selectedAccount.username} direset ke "password123"`,
      });
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (filterConfig.role !== "all") {
      processed = processed.filter(
        (a) => a.role.toLowerCase() === filterConfig.role
      );
    }
      ...
      <section className="w-full max-w-[1076px] space-y-6">
        ...
        <div className="flex flex-col items-center text-center gap-2 mt-2">
          ...
          <p className="text-[#6B7280]">
            Kelola dan tambah akun untuk Groundcrew dan Warehouse
          </p>
        </div>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["all", "supervisor", "warehouse", "groundcrew"].map(
                      (r) => (
                        <button
                          key={r}
                          onClick={() =>
          ...
          <div className="divide-y divide-[#E9EEF3]">
            {pageLoading ? (
              ...
              filteredAccounts.map((row, index) => (
                <div
                  key={`account-${row.id}-${index}`}
                  className="px-4 py-4 sm:py-5 flex items-center hover:bg-gray-50/50 transition"
                >
                  <div className="flex-1 flex items-center gap-3 pr-4">
                    <div
                      className={`relative h-10 w-10 rounded-full flex items-center justify-center text-white text-sm font-semibold ${
                        row.is_active
                    ...
                    >
                      ...
                      {!row.is_active && (
                        <div
                          className="absolute bottom-0 right-0 w-3 h-3 bg-red-500 border-2 border-white rounded-full"
                          title="Non-Aktif"
                        ></div>
                      )}
                    </div>
                    <div className="leading-tight">
                      <div className="text-[17px] font-medium text-[#111827]">
                        {row.username}
                      </div>
                  ...
                  <div className="w-28 sm:w-44 flex justify-end gap-2 sm:gap-3">
                    ...
                    >
                      <svg
                        ...
                        strokeWidth="1.5"
                      >
                        <path d="M10.6667 6.66667V5.33333C10.6667 3.86058 9.47276 2.66667 8 2.66667C6.52724 2.66667 5.33333 3.86058 5.33333 5.33333V6.66667M3.33333 6.66667H12.6667C13.0349 6.66667 13.3333 6.96514 13.3333 7.33333V12.6667C13.3333 13.0349 13.0349 13.3333 12.6667 13.3333H3.33333C2.96514 13.3333 2.66667 13.0349 2.66667 12.6667V7.33333C2.66667 6.96514 2.96514 6.66667 3.33333 6.66667Z" />
                      </svg>
                    </button>
                    <button
                      type="button"
        ...
        createPortal(
          <div className="fixed inset-0 z-[999] flex items-center justify-center bg-[#050022]/40 px-4 backdrop-blur-sm overflow-y-auto">
            <div className="w-full max-w-sm rounded-[32px] bg-white p-6 sm:p-8 text-center shadow-xl">
              <h2 className="text-2xl font-semibold text-[#11264D]">
                Hapus Akun?
              </h2>
              {selectedAccount && (
              ...
              <div className="mt-8 flex gap-4 justify-center">
                <button
                  ...
                  className="w-32 rounded-full border border-[#FF3B30] px-6 py-2 text-sm font-semibold text-[#FF3B30] hover:bg-red-50"
                >
                  {deleteLoading ? "..." : "Hapus"}
                </button>
                <button
                  ...
                  disabled={deleteLoading}
                  className="w-32 rounded-full bg-[#0D63F3] px-6 py-2 text-sm font-semibold text-white hover:bg-blue-700"
                >
                  Batal
                </button>
              </div>
            ...
            <div className="w-full max-w-sm rounded-[32px] bg-white p-6 sm:p-8 text-center shadow-xl">
              ...
              <div className="mt-8 flex gap-4 justify-center">
                <button
                  ...
                  className="w-32 rounded-full border border-amber-500 px-6 py-2 text-sm font-semibold text-amber-600 hover:bg-amber-50"
                >
                  {resetLoading ? "..." : "Ya, Reset"}
                </button>
                <button
