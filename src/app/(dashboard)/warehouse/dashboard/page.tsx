...
const RequestSummaryTable: React.FC<{
  data: { documentName: string; quantity: number }[];
  onMore?: () => void;
}> = ({ data, onMore }) => (
  <>
...
export default function WarehouseDashboardPage() {
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!ignore)
          setWelcome((dashboardRes as { message?: string })?.message ?? null);

        // Single API call for both requests and history data
        const response = await skybase.warehouseRequests.list();
        if (!ignore && response.data) {
          const responseData = response.data;
          let allRequests: WarehouseRequest[] = [];
          ...
          if (Array.isArray(responseData)) {
            ...
            !("wh_req_id" in responseData)
          ) {
            allRequests = (responseData as { items: WarehouseRequest[] }).items;
          }

          // Filter pending for requests display
          setRequestsData(allRequests.filter((r) => r.status === "PENDING"));
          // Use full list for history
          setHistoryData(allRequests);
        }
  ...
  const historyTableData: HistoryRow[] = historyData.slice(0, 10).map((req) => {
    const firstItem = req.items?.[0] as RequestItem | undefined;
    const itemName =
      firstItem?.item?.name || `Item #${firstItem?.item_id || req.wh_req_id}`;
    return {
      id: req.wh_req_id,
      document: itemName,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        <section className="md:col-span-2 p-0">
          ...
          <RequestSummaryTable
            ...
            onMore={handleRequestSelengkapnya}
          />
        </section>
            ...
            <WhiteCard className="overflow-hidden">
              ...
              <div className="divide-y divide-[#E9EEF3]">
                {historyTableData.map((row) => (
                  ...
                  >
                    <span className="truncate font-medium" title={row.document}>
                      {row.document}
                    </span>
