...
export default function FlightReportView({ role }: FlightReportViewProps) {
  ...
  const [downloadingSectionId, setDownloadingSectionId] = useState<
    string | null
  >(null);
  ...
  const handleDownloadRange = async () => {
    ...
    try {
      ...
      if (pdfSections.length === 0) {
        setNotification({
          type: "error",
          message: "Tidak ada data penerbangan untuk direkap.",
        });
        return;
      }
      ...
      const rolePrefix =
        role === "groundcrew" ? "GC-" : role === "warehouse" ? "WH-" : "";
      generateRecapPDF(
        recapData,
        `${rolePrefix}Rekap-Laporan-${startDate || "All"}-to-${
          endDate || "All"
        }.pdf`
      );
      setNotification({
        type: "success",
        message: "Rekap laporan berhasil diunduh",
      });
    } catch (e) {
      console.error(e);
      setNotification({
        type: "error",
        message: "Gagal mengunduh rekap laporan",
      });
    } finally {
  ...
  const handleDownloadSection = async (section: ReportSectionUI) => {
    ...
    try {
      ...
      generateRecapPDF(
        recapData,
        `${rolePrefix}Laporan-Harian-${section.id}.pdf`
      );
      setNotification({
        type: "success",
        message: `Laporan harian ${section.title} berhasil diunduh.`,
      });
    } catch (error) {
      console.error(error);
      setNotification({
    ...
    try {
      ...
      const reportDataMock: Partial<AircraftStatusReport> = {
        ...
        current_inventory: {
          ...
          expiring_soon: 0,
          by_category: {
            ASE: { total: 0, valid: 0, expired: 0, expiring_soon: 0 },
            DOC: { total: 0, valid: 0, expired: 0, expiring_soon: 0 },
          },
        },
        timeline: {},
      };
      ...
      const rolePrefix =
        role === "groundcrew" ? "GC-" : role === "warehouse" ? "WH-" : "";
      generateStatusReportPDF(
        reportDataMock as AircraftStatusReport,
        `${rolePrefix}Laporan-${schedule.registration}.pdf`,
        items
      );
